...
function EvidenceGraph() {
  ...
  useEffect(() => {
    if (activeNodeId) {
      const key = activeNodeId as keyof typeof EVIDENCE_MAP;
      const evidence = EVIDENCE_MAP[key];
      if (evidence) {
        navigate({ to: "/app/evidence", search: { doc: evidence.id, ev: evidence.id } });
      }
    }
  ...
  return (
    <div className="rounded-lg border border-border/50 bg-background/80 p-4">
      ...
      >
        ...
        <g cursor="pointer" onClick={() => setActiveNodeId("center")}>
          <circle
            cx="310"
            cy="220"
            r={NODE_RADIUS}
          ...
          <text x="310" y="248" textAnchor="middle" className="fill-muted/60 text-[10px]">
        ...
        {CONNECTIONS.map((conn) => {
          const evidence = EVIDENCE_MAP[conn.evidenceId as keyof typeof EVIDENCE_MAP];
          const angleRad = (conn.angle * Math.PI) / 180;
          const cx = 310 + 270 * Math.cos(angleRad);
          const cy = 220 + 270 * Math.sin(angleRad);

          return (
            <g key={conn.key} onClick={() => setActiveNodeId(evidence.id)}>
              <circle
                cx="0"
                cy="0"
                r="8"
              ...
              <text x="0" y="18" textAnchor="middle" className="fill-muted/60 text-[8.5px]">
          ...
          <g>
            ...
            >
              {activeNodeId === "center"
                ...
                : EVIDENCE_MAP[activeNodeId as keyof typeof EVIDENCE_MAP]?.title || ""}
            </text>
            <text x="110" y="-55" textAnchor="middle" className="fill-muted/60 text-[10px]">
              Type:{" "}
              {activeNodeId === "center"
                ? "—"
                : EVIDENCE_MAP[activeNodeId as keyof typeof EVIDENCE_MAP]?.type || ""}{" "}
              | Page:{" "}
              {activeNodeId === "center"
                ? "—"
                : EVIDENCE_MAP[activeNodeId as keyof typeof EVIDENCE_MAP]?.page || 0}{" "}
              | Confidence:{" "}
              {activeNodeId === "center"
                ? "—"
                : (EVIDENCE_MAP[activeNodeId as keyof typeof EVIDENCE_MAP]?.confidence * 100) | 0}
              % | Verification:{" "}
              {activeNodeId === "center"
                ? "—"
                : EVIDENCE_MAP[activeNodeId as keyof typeof EVIDENCE_MAP]?.verification || ""}
            </text>
          </g>
        )}
      </svg>
