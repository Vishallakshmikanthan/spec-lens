...
import type { ComponentIntel, EvidenceType } from "@/types/speclens";

export const Route = createFileRoute("/app/components/$mpn")({
  component: ComponentIntelPage,
});
...
function ComponentIntelPage() {
  const { mpn } = Route.useParams();
  const [c, setC] = useState<ComponentIntel | undefined>(undefined);

  useEffect(() => {
    api.getComponent(mpn).then(setC);
  }, [mpn]);

  if (!c) {
    return (
      <div className="panel p-8 text-center">
        <h2 className="font-mono text-[18px] font-semibold">{mpn}</h2>
        <p className="text-[13px] text-muted-foreground">Loading component intelligence…</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="space-y-6 px-4 py-6 sm:px-6">
        <div className="relative max-w-md">
          ...
          <input
            value={q}
            onChange={(e) => setC({ ...c, mpn: e.target.value })}
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="mt-4 grid gap-4 lg:grid-cols-2">
            <Section title="Specifications">
              <dl className="divide-y divide-border overflow-hidden rounded-lg border border-border">
                {c.specs.map((s: { label: string; value: string }) => (
                  <div
                    key={s.label}
            ...
            <div className="space-y-4">
              ...
              <Section title="Verified evidence">
                <ul className="grid grid-cols-2 gap-2">
                  {c.verified.map((v: { type: EvidenceType; label: string; ok: boolean }) => (
                    <li
                      key={v.label}
          ...
          <TabsContent value="related" className="mt-4">
            <ul className="divide-y divide-border overflow-hidden rounded-lg border border-border">
              {c.related.map((r: { mpn: string; note: string }) => (
                <li
                  key={r.mpn}
          ...
          <TabsContent value="history" className="mt-4">
            <ul className="space-y-2">
              {c.history.map((h: { at: string; event: string }) => (
                <li
                  key={h.at}
