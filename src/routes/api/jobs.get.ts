...
import { processingJobs, workspaces, users, workspaceMembers } from "@/database/schema";
...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    const workspaceCondition = eq(processingJobs.workspaceId, activeWorkspace);
    ...
    if (status) {
      jobsQuery = jobsQuery.where(eq(processingJobs.status, status));
    }

    // 6. Apply limit and order by most recent first
    const allJobs = await jobsQuery
      .orderBy(processingJobs.startedAt.descnullsLast)
      .limit(limit);
