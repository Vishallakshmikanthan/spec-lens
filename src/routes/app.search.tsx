...
import { ConfidenceBar, EvidenceTypeBadge, VerificationBadge, evidenceIcon } from "@/components/speclens/evidence-ui";
...
export const Route = createFileRoute("/app/search")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Search datasheets in natural language and retrieve ranked, verified visual evidence regions." },
      { property: "og:title", content: "Visual Search — SpecLens" },
      { property: "og:description", content: "Search datasheets in natural language and retrieve ranked, verified visual evidence regions." },
...
function VisualSearch() {
  ...
  useEffect(() => {
    ...
    let alive = true;
    setLoading(true);
    api.search(q, { types, minConfidence: minConf }).then((r) => { if (alive) { setData(r); setLoading(false); } });
    return () => { alive = false; };
  }, [q, types, minConf]);
  ...
  return (
    <div>
      <PageHeader title="Visual Search" subtitle="Retrieve verified evidence regions across every indexed datasheet." />
      <div className="space-y-6 px-4 py-6 sm:px-6">
        <form onSubmit={(e) => { e.preventDefault(); void navigate({ to: "/app/search", search: { q: input } }); }} className="relative">
          <Search className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
          <input value={input} onChange={(e) => setInput(e.target.value)} aria-label="What are you looking for?"
        ...
        <div className="flex flex-wrap gap-2">
          {searchExamples.map((s) => (
            <button key={s} onClick={() => void navigate({ to: "/app/search", search: { q: s } })}
          ...
          <aside className="space-y-4">
            <div className="panel p-3">
              <h2 className="mb-2 flex items-center gap-1.5 font-mono text-[10.5px] uppercase tracking-[0.14em] text-muted-foreground">
                <SlidersHorizontal className="size-3" aria-hidden="true" />Evidence type
              </h2>
              <ul className="space-y-0.5">
                {(Object.keys(EVIDENCE_TYPE_LABEL) as EvidenceType[]).map((t) => {
                  ...
                  return (
                    <li key={t}>
                      <button onClick={() => toggle(t)} aria-pressed={on}
                        className={cn("flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[12.5px] transition-colors hover:bg-secondary",
                          on && "bg-primary/10 text-foreground")}>
                        <Icon className={cn("size-3.5", on ? "text-primary" : "text-muted-foreground")} aria-hidden="true" />
            ...
            <div className="panel p-3">
              <label htmlFor="conf" className="font-mono text-[10.5px] uppercase tracking-[0.14em] text-muted-foreground">
                Min confidence · {Math.round(minConf * 100)}%
              </label>
              <input id="conf" type="range" min={0} max={99} value={minConf * 100}
                onChange={(e) => setMinConf(Number(e.target.value) / 100)} className="mt-3 w-full accent-primary" />
            </div>
          </aside>
          ...
          <div className="space-y-3">
            {loading && (
              <p className="flex items-center gap-2 text-[13px] text-muted-foreground">
                <Loader2 className="size-4 animate-spin" aria-hidden="true" />Retrieving evidence…
              </p>
            )}
            {!loading && !q && (
              <EmptyState icon={Search} title="Start with a question." description="Ask in natural language or search by MPN."
                suggestions={["LM358 pinout", "package dimensions", "timing diagram"]} />
            )}
            {!loading && q && data && data.total === 0 && (
              <EmptyState icon={Search} title="No evidence matched this query."
            ...
            {!loading && data && data.total > 0 && (
              <>
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <span className="font-mono text-[11px] text-muted-foreground">{data.latencyMs} ms · demo index</span>
                </div>
                <ul className="space-y-3">
                  {data.results.map((e, i) => (
                    <li key={e.id} className="panel animate-rise grid gap-4 p-4 sm:grid-cols-[130px_1fr]" style={{ animationDelay: `${i * 40}ms` }}>
                      ...
                      <div className="min-w-0 space-y-2">
                        ...
                        <p className="text-[11.5px] text-muted-foreground">Matched using: {e.matchedBy.join(" · ")}</p>
                        <div className="flex flex-wrap gap-2 pt-1">
                          <Button asChild size="sm" variant="secondary">
                            <Link to="/app/evidence" search={{ doc: e.documentId, ev: e.id }}>Inspect Evidence</Link>
                          </Button>
                          <Button asChild size="sm" variant="outline">
                            <Link to="/app/evidence" search={{ doc: e.documentId, ev: e.id }}>Open Page</Link>
                          </Button>
