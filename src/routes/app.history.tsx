...
export const Route = createFileRoute("/app/history")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Review, re-run and manage previous evidence searches across your datasheets.",
      },
      ...
      <div className="space-y-4 px-4 py-6 sm:px-6">
        {items.length === 0 ? (
          <EmptyState
            ...
            description="Run a visual search to build your history."
            action={
              ...
              <li key={h.id} className="flex flex-wrap items-center gap-3 bg-surface px-3 py-3">
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block font-mono text-[11px] text-muted-foreground">
                    {h.mpn} · {h.results} results · best {(h.bestConfidence * 100).toFixed(1)}% ·{" "}
                    {h.at}
                  </span>
                </span>
                <Button asChild size="sm" variant="secondary">
                  <Link to="/app/search" search={{ q: h.query }}>
