  ...
  head: () => ({
    meta: [
      { title: "Developer Console — SpecLens" },
      {
        name: "description",
        content:
          "Inspect the SpecLens API contract: endpoints, requests, responses, latency and evidence IDs.",
      },
...
const endpoints = [
  {
    method: "POST",
    path: "/api/search",
    body: { query: "find pinout", mpn: "LM358", evidence_type: "pinout" },
  },
...
function DeveloperPage() {
  ...
  const run = async () => {
    ...
    let data: unknown;
    if (ep.path.startsWith("/api/search"))
      data = await api.search("find pinout", { types: ["pinout"] });
    ...
    else if (ep.path === "/api/copilot")
      data = await api.askCopilot("What is the supply voltage range?");
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="The API contract the SpecLens frontend is built against."
      />
          ...
          {endpoints.map((e, i) => (
            <li key={e.path}>
              <button
                ...
                aria-pressed={i === idx}
                className={cn(
                  "flex w-full items-center gap-2 px-4 py-3 text-left font-mono text-[12px] hover:bg-surface",
                  i === idx && "bg-surface",
                )}
              >
                <span
                  className={cn(
                    "w-11 shrink-0 text-[10.5px]",
                    e.method === "POST" ? "text-warning" : "text-success",
                  )}
                >
                  {e.method}
                </span>
