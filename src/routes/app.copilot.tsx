...
export const Route = createFileRoute("/app/copilot")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Ask questions grounded in retrieved technical evidence, with citations and confidence." },
      { property: "og:title", content: "SpecLens Copilot — SpecLens" },
      { property: "og:description", content: "Ask questions grounded in retrieved technical evidence, with citations and confidence." },
...
function CopilotPage() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)] flex-col">
      <PageHeader title="SpecLens Copilot" subtitle="Ask questions grounded in retrieved technical evidence." />
      <div className="mx-auto w-full max-w-3xl flex-1 space-y-5 px-4 py-6 sm:px-6">
            ...
            {m.role === "user" ? (
              ...
              <div className="panel p-4">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="font-mono text-[10.5px] uppercase tracking-[0.14em] text-muted-foreground">Grounded answer</span>
                  {m.confidence && (
                    <span className="ml-auto font-mono text-[11px] text-success">{(m.confidence * 100).toFixed(1)}%</span>
                  )}
                </div>
                <p className="text-[13.5px] leading-relaxed">{m.content}</p>
                {m.sources?.length ? (
                  <div className="mt-4 border-t border-border pt-3">
                    <p className="mb-2 font-mono text-[10.5px] uppercase tracking-[0.14em] text-muted-foreground">Sources</p>
                    <ul className="space-y-1.5">
                      {m.sources.map((s) => (
                        <li key={s.evidenceId}>
                          <Link to="/app/evidence" search={{ doc: undefined, ev: s.evidenceId }}
                            ...
                            <span className="ml-auto shrink-0 font-mono text-[11px] text-muted-foreground">p{s.page} · {(s.confidence * 100).toFixed(1)}%</span>
                          </Link>
                        </li>
                      ))}
                    </ul>
        ...
        {thinking && (
          <p className="flex items-center gap-1.5 text-[13px] text-muted-foreground" aria-live="polite">
            Retrieving evidence
            <span className="flex gap-1">
              {[0, 1, 2].map((i) => (
                <span key={i} className="size-1 animate-bounce rounded-full bg-primary" style={{ animationDelay: `${i * 120}ms` }} />
              ))}
            </span>
          </p>
        )}
      ...
      <div className="sticky bottom-0 border-t border-border bg-background/90 p-4 backdrop-blur">
        <form onSubmit={send} className="mx-auto flex max-w-3xl gap-2">
          <input value={input} onChange={(e) => setInput(e.target.value)} aria-label="Ask SpecLens Copilot"
